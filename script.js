...
function canvasArtwork2() {
  ...
  function render() {
    context.canvas.style.opacity = (imageSeq.frame * 2) / totalImages;
    scaleImage(images[imageSeq.frame], context);
  }
...
function effect3() {
  var clutter = "";
  document
    .querySelector(".page6>.heading>.heading-para")
    .textContent.split(" ")
    .forEach(function (data) {
      clutter += `<span> ${data}</span>`;

      document.querySelector(".page6>.heading>.heading-para").innerHTML =
        clutter;
    });
  gsap.to(".page6 span", {
    scrollTrigger: {
